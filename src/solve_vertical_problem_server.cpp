         ...
         rendezvous_problem::vertical_problem::Response &res)
{
  ...
  for (int t=0; t<T+1; ++t)
  {
    ...
    // params.b[t][0] = (req.dist_traj.data[t] <= 2.0) ? 1.0 : 0.0;
  }
...
{
  ros::init(argc, argv, "vertical_problem_server");
  ros::NodeHandle n;
  ...
  ros::ServiceServer service = n.advertiseService("vertical_problem", get_solution);
  ROS_INFO("Ready to solve vertical problem");
  ros::spin();
