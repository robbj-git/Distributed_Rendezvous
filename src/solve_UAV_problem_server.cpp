#include "ros/ros.h"
#include "rendezvous_problem/UAV_problem.h"
extern "C"{
  #include "UAV/solver.h"
}
...
bool get_solution(rendezvous_problem::UAV_problem::Request  &req,
         rendezvous_problem::UAV_problem::Response &res)
{
  ...
  int stride = req.xb_traj.array.layout.dim[1].stride;
  for(int t=0; t<T+1; ++t)
  {
    for(int i=0; i<n; ++i)
    {
      params.xb[t][i] = req.xb_traj.array.data[stride*t + i];
    }
  }
...
{
  ros::init(argc, argv, "UAV_problem_server");
  ros::NodeHandle n;
  ...
  ros::ServiceServer service = n.advertiseService("uav_problem", get_solution);
  ROS_INFO("Ready to solve UAV problem");
  ros::spin();
